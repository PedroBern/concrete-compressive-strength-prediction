...
```{r join-90-91-100, echo=F, eval=F}
...
# Table - First 6 samples after reorganization/EN>
...
# Table - Descriptive statistics - continuous variables/EN>
summ <- t(
...
# Figure - Descriptive statistics - categorical variables/EN>
...
# Figure - Correlation grouped by age/EN>
...
# Figure - Relationship between approximated mix, water, MPa and age/EN>
...
mix_dat <- mix_dat %>%
  gather("day", "mpa", -c(mix_app, `water_/_cement`,
                          ...
                          `coarse_aggregate_/_cement`)) %>%
  drop_na()
mix_dat$day <- factor(mix_dat$day)
...
# Figure - Relationship between concrete main features/EN>
...
# Figure - Variables distribution/EN>
...
# Figure - Variables distribution grouped by age/EN>
...
# Figure - Principal component analysis on ingredients/EN>
...
dens <- function(d, n){
  dens <- full_join(
      d$train %>%
        select(y) %>%
        mutate(Grup = g1, day = str_sub(n, 2)),
      d$test %>%
        select(y) %>%
        mutate(Grup = g2, day = str_sub(n, 2))
    )
  dens
}
...
densities %>%
  ggplot(aes(y, fill = Grup)) +
  geom_density(alpha = 0.5) +
  facet_wrap(~dia_f, ncol=3) +
  xlab("MPa") +
  ylab(ylabel) +
  theme_bw() +
  theme_minimal() +
