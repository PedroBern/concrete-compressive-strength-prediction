---
title: "Concrete compressive strength prediction with machine learning"
...
abstract: Compressive strength is the main characteristic of concrete. This work used
  a set of samples widely used to build 6 forecast models of this characteristic,
  each one specifically for an age, 3, 7, 14, 28, 56 and 100 days from 7 ingredients
  (water, cement, fine aggregate, coarse aggregate, fly ash, blast furnace slag, and
  superplasticizers) and 6 relationships between the main ingredients (water/cement,
  coarse aggregate/cement, fine aggregate/cement, fine aggregate / coarse aggregate,
  water / coarse aggregate and water/fine aggregate) to make the predictions. The
  algorithm *Parallel Random Forest* was used (*parRF* of the *caret* package in *R*)
  . The results were satisfactory, being the best for the 3-day model (*RMSE* of 3.31)
  and the worst for the 100-day model (*RMSE* of 5.85). The 28-day model had *RMSE*
  of 4.72. The work indicated that the separation in a model for each age is promising
  and was able to generate compatible results or better than other studies, opening
  way for new models creation following this proposal, but using different algorithms
  or the combination of different algorithms.
---
...
# Tabela - Samples with same composition
same_samples <- dat %>%
...
# Tabela - Same samples with different results
same_samples_2 <- dat %>%
...
# Tabela - Previous samples after processing
same_samples <- dat %>%
...
### Table - First 6 samples of train data of the 28 days model {#slug}
