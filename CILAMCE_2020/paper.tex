...
Compressive strength is the main characteristic of concrete, measured by tests of international standards that consist of breaking of specimens \cite{Gambhir1990}. Measurement when the concrete is 28 days old is mandatory and represents the grade of the concrete. Knowing in advance the result for a given age, based on the proportions of its ingredients, is of great interest to concrete manufacturers, construction companies, and civil engineers.
 
The compressive strength is a nonlinear function of its ingredients and age, making it difficult to establish an analytical formula, although some formulas have already been proposed by \citet{Hasan2011} and  \citet{Kabir2012}. However, most of the studies have built models including the age as a feature along with the ingredients, but due to the non linearity between the compressive strength and age, we have found the need of further investigation of models that separate the age and analyse only the ingredients, then specify for each age. 
...
\citet{Alshamiri2020} proposed a new Regularized Extreme Learning Machine (RELM) method to train Artificial Neural Networks models to predict the compressive strenght. The results were compared with several known algorithms running on the same dataset, including individual and ensembles, and the proposed model had the best result by far.

\citet{Hameed2020} compares Artificial Neural Network models with Multiple Linear Regression to predict the compressive strength force and have found that Artificial Neural Network models obtain much more accuracy than the Multiple Linear Regression.
...
The methodology was carried out using RStudio software \cite{RStudio}, an integrated virtual environment for code development in R language \cite{RCore}. Throughout the process, all code executed was documented in the same order as its execution and pushed to the github repository \cite{github}. The repository contains an extended version of this paper, including all the code, required packages and versions. In order to guarantee reproducibility, whenever there was code that uses probabilistic operations, a seed was defined before its execution, ensuring results consistency when running on another machine.

% --------------------------------------------------------------------------
\subsection{Dataset} \label{subsec:dataset}
% --------------------------------------------------------------------------
The data was downloaded from the University of California Irvine website \cite{downloadData}. In total there are 1030 rows with 9 columns. Each row represents a sample with the variables: compressive strength, age, and 7 ingredients (water, cement, fine aggregate, coarse aggregate, fly ash, blast furnace slag, and superplasticizers).
...
\item Ages of 90, 91, and 100 days were merged into a single 100-day category. This step was taken with the following development: first, plotting and analysing the boxplot grouped by age in Fig. \ref{fig:boxplot} showed that samples of 90, 91 and 100 days have distinct concentrations of compressive strength values. Then a principal component analysis (PCA) of the ingredients were made in Fig. \ref{fig:pca}, showing that the ingredients combination of these ages are very distinct. As they are very close ages, it is reasonable that we can join these ages without any prejudice to the predictions. Finally, the 100-day mark was chosen because the analysis of samples that had data on at least five different ages showed that the compressive strength tends to increase through time. Therefore, an older age tends to provide more conservative results.
...
\item For samples with the same ID and the same age, but with different values of compressive strength, the data was combined into a single row for each combination of ID and age, containing an average of the compressive strengths;

\item Only the IDs that have data on the 28-day mark were kept;
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[scale=0.9]{Figures/boxplot.pdf}
\vspace{12pt}
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[scale=0.9]{Figures/pca-90-91-100.pdf}
\vspace{12pt}
...
After these manipulations, the final number of samples was reduced from 1030 rows to 916 rows, with 416 different ingredient configurations (IDs). A xls file of the data at this point is available for download at the github repository \cite{github}.
...
In addition to the already presented plots, several other plots and tables were built to perform the exploration and visualization of the samples prepared in the previous step, including:

\begin{itemize}

\item Analysis of the descriptive statistics of the continuous and categorical variables;

\item Distribution of the variables in relation to the compressive strength;

\item Correlation between the variables grouped by age;

\item Relationship between the approximate mix and the compressive strength;

\item Relationship between the main concrete ingredients and the compressive strength;

\item Principal component analysis (PCA) of the ingredients.

\end{itemize}

The plots for the statistical analysis of the categorical variables are presented, in the Fig. \ref{fig:categorical}.

\begin{figure}[!ht]
\begin{center}
\includegraphics[scale=0.8]{Figures/stat-summ-categorical.pdf}
\vspace{12pt}
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[scale=0.9]{Figures/dist-split.pdf}
\vspace{12pt}
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[scale=0.9]{Figures/results-comparison.pdf}
\vspace{12pt}
